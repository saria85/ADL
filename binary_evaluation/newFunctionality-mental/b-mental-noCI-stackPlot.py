...
data = pd.read_csv('C:/Users/M193053/PycharmProjects/ADL-distribution/noCIFunctionality-final-withdiagdate.csv',names=names)

data['year'] = data['Answer Date'].str.split('/').str[2]
data['month'] = data['Answer Date'].str.split('/').str[0]

data['diagyear'] = data['date'].str.split('/').str[2]
data['diagmonth'] = data['date'].str.split('/').str[0]

data['Answer Date'] = pd.to_datetime(data['Answer Date'],errors='coerce')
data['date'] = pd.to_datetime(data['date'],errors='coerce')
data = data.dropna()
data['6month'] = (pd.np.ceil((data['date']-pd.Timedelta(days=1)-data['Answer Date'])
...
plt.title('nonCognitive Impairement-Stack bar')
plt.savefig('C:/Users/M193053/PycharmProjects/ADL-distribution/binary_evaluation/binary-files-plot/newFunctionality-mental/binary-stack-mental-noCI.png')
plt.show()
